...
def get_move(player, board):
  ...
  for i in valid_moves:
      a = bestMove(board,i,3,True,player)
      if a > max:
        max = a
        indexMax = index
      index+=1
  return valid_moves[indexMax]
...
def bestMove(board,move,search_depth,myTurn,player):
  if player == 1:
    enemy = 2
  else:
    enemy = 1
  ...
  if myTurn:
    ...
    valid_moves = get_valid_moves(enemy, newBoard)
    for i in valid_moves:
      a=bestMove(newBoard,i,search_depth-1,False,enemy)
    ...
    valid_moves = get_valid_moves(enemy,newBoard)
    for i in valid_moves:
      a = bestMove(newBoard,i,search_depth-1,True,enemy)
      minScore=min(a,minScore)
    return minScore
